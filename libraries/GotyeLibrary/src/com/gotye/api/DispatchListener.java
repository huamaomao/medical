...
public class DispatchListener {
	...
	static void onListener(GotyeEventCode code, int statusCode, Object... param) {
		GotyeDelegate mListener = GotyeAPI.mListener;
		switch (code) {
		...
		case GotyeEventCodeSearchUserList:
			if (mListener != null) {
				mListener.onSearchUserList(statusCode,
						(List<GotyeUser>) param[0],(List<GotyeUser>) param[1], (Integer) param[2]);
			}
			break;
		...
		case GotyeEventCodeGetHistoryMessageList:
			...
			if (mListener != null) {
				mListener.onGetMessageList(statusCode,
		...
		case GotyeEventCodeKickoutUser:
			if (mListener != null) {
				mListener.onKickoutGroupMember(statusCode, (GotyeGroup) param[0],(GotyeUser) param[1]);
			}
			break;

		case GotyeEventCodeChangeGroupOwner:
			if (mListener != null) {
				mListener.onChangeGroupOwner(statusCode, (GotyeGroup) param[0],(GotyeUser) param[1]);
			}
			break;
		...
		case GotyeEventCodeDownloadMedia:
			if (mListener != null) {
				mListener.onDownloadMedia(statusCode, (GotyeMedia) param[0]);
			}
			break;
		...
		case GotyeEventCodeReconnecting:
			if (mListener != null) {
				mListener.onReconnecting(statusCode,(GotyeUser) param[0]);
			}
			break;
		case GotyeEventCodeCustomerService:
			if (mListener != null) {
				mListener.onGetCustomerService(statusCode, (GotyeUser) param[0], (Integer)param[1], (String)param[2]);
			}
			break;
		default:
			break;
